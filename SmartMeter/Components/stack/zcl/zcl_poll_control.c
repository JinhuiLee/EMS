...
ZStatus_t zclPollControl_RegisterCmdCallbacks( uint8 endpoint, zclPollControl_AppCallbacks_t *callbacks )
{
  ...
  pNewItem = osal_mem_alloc( sizeof( zclPollControlCBRec_t ) );
  if ( pNewItem == NULL )
  {
    return ( ZMemError );
  }
...
/*********************************************************************
 * @fn      zclPollControl_CountClients
 *
 * @brief   Determine # of clients for the PollControl cluster from this endpoint
 *
 * @param   srcEP     - application endpoint
 * @param   clusterID - usually ZCL_CLUSTER_ID_GEN_POLL_CONTROL (0x0020)
 *
 * @return  # of entries from this source endpoint to this cluster
 */
uint8 zclPollControl_CountClients( uint8 srcEP, uint16 clusterID )
{
  uint8 count = 0;
  uint16 x;   // index into binding table entry
  uint8  i;   // index into cluster list
  BindingEntry_t *pBind;

  // couldn't use bindNumBoundTo() because it didn't check cluster ID
  for ( x = 0; x < gNWK_MAX_BINDING_ENTRIES; x++ )
  {
    pBind = &BindingTable[x];
    if ( pBind->srcEP == srcEP )
    {
      for ( i = 0; i < gMAX_BINDING_CLUSTER_IDS; i++ )
      {
        if ( pBind->clusterIdList[i] == clusterID )
          ++count;
      }
    }
  }
