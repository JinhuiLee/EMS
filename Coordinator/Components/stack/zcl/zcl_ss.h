...
#define COMMAND_SS_IAS_ACE_ZONE_STATUS_CHANGED                           0x07
#define COMMAND_SS_IAS_ACE_PANEL_STATUS_CHANGED                          0x08
...
typedef struct
{
  uint8   zoneID;
  uint16  zoneType;
  uint8   zoneAddress[Z_EXTADDR_LEN]; // pointer to array of 8 bytes
} IAS_ACE_ZoneTable_t;
...
{
  uint8   armMode;
  uint8  *pArmDisarmCode;  // pointer to array of 8 bytes (UTF-8 String)
  uint8   zoneID;
} zclCmdSSIASACEArmPayload_t;

/*** ZCL IAS ACE Cluster: BYPASS Cmd payload ***/
typedef struct
{
  uint8   numZones;
  uint8  *pZoneID;  // pointer to array of variable length
} zclCmdSSIASACEBypassPayload_t;

/*** ZCL IAS ACE Cluster: GET_ZONE_INFORMATION cmd payload: ***/
typedef uint8 zclCmdSSIASACEGetZoneInformationPayload_t;

/*** ZCL IAS ACE Cluster: Zone Status Changed Cmd payload ***/
typedef struct
{
  uint8   zoneID;
  uint16  zoneStatus;
  uint8  *pZoneLabel;  // pointer to array of 16 bytes (UTF-8 String)
} zclCmdSSIASACEZoneStatusChangedPayload_t;

/*** ZCLE IAS ACE Cluster: Panel Status Changed Cmd payload ***/
typedef struct
{
  uint8 panelStatus;
  uint8 secondsRemaining;
} zclCmdSSIASACEPanelStatusChangedPayload_t;
...
typedef struct
{
  uint8  zoneID;
  uint16 zoneType;
  uint8 *ieeeAddress; // pointer to array of variable length
...
typedef uint8 (*zclSS_ACE_Arm_t)( zclCmdSSIASACEArmPayload_t *pCmd );
...
// This callback is called to process an Arm command
typedef ZStatus_t (*zclSS_ACE_ZoneStatusChanged_t)( zclCmdSSIASACEZoneStatusChangedPayload_t *pCmd );

// This callback is called to process an Arm command
typedef ZStatus_t (*zclSS_ACE_PanelStatusChanged_t)( zclCmdSSIASACEPanelStatusChangedPayload_t *pCmd );
...
typedef ZStatus_t (*zclSS_WD_Squawk_t)( zclCmdSSWDSquawkPayload_t *pCmd );
...
extern ZStatus_t zclSS_IAS_Send_ZoneStatusEnrollResponseCmd( uint8 srcEP, afAddrType_t *dstAddr,
  ...
  *      pArmDisarmCode - (64 bits) arm/disarm code for the system
  *      zoneID - index to the zone table of the CIE
  */
...
extern ZStatus_t zclSS_Send_IAS_ACE_GetZoneInformationCmd( uint8 srcEP, afAddrType_t *dstAddr,
  ...
  *      zoneID - index to the zone table of the CIE
  *      zoneStatus - current value of the attribute
  *      pZoneLabel - the first 16 bytes of attribute programmed into the AES client
  */
extern ZStatus_t zclSS_Send_IAS_ACE_ZoneStatusChangedCmd( uint8 srcEP, afAddrType_t *dstAddr,
  ...
  *      panelStatus - current value of the attribute
  *      secondsRemaining - time left for server to be in state indicated by panelStatus
  */
